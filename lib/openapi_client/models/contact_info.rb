  ...
  class ContactInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ContactInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ContactInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@telephone.nil? && @telephone.to_s.length < 1
        invalid_properties.push('invalid value for "telephone", the character length must be great than or equal to 1.')
      end

      if !@fax.nil? && @fax.to_s.length < 1
        invalid_properties.push('invalid value for "fax", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@telephone.nil? && @telephone.to_s.length < 1
      return false if !@fax.nil? && @fax.to_s.length < 1
    ...
    def fax=(fax)
      if !fax.nil? && fax.to_s.length < 1
        fail ArgumentError, 'invalid value for "fax", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [telephone, website, fax, email].hash
    end
