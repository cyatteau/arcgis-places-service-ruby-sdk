...
module OpenapiClient
  # A pair of times defining the start and end of a time period.   For example, this could define opening hours or popular hours. Hours are shown in 24-hour time in the local timezone of the place or POI.  Where a time range is 24-hours (for example a venue that is open 24-hours),  the `from` property will be 00:00 and the `to` property will be 23:59. 
  class TimeRange
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TimeRange` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TimeRange`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @from !~ Regexp.new(/^\d{2}:\d{2}$/)
      return false if @to.nil?
      return false if @to !~ Regexp.new(/^\d{2}:\d{2}$/)
      true
    end
