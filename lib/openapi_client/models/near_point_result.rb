...
module OpenapiClient
  # A result of searching for places using a `places/near-point` request.  The result object includes a single place that satisfied the search and the distance from the search point, in meters. 
...
#This place Id can be passed to the `places/{placeId}` endpoint to retrieve additional details.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::NearPointResult` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::NearPointResult`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @distance < 0
        invalid_properties.push('invalid value for "distance", must be greater than or equal to 0.')
      end
    ...
    def distance=(distance)
      if distance.nil?
        fail ArgumentError, 'distance cannot be nil'
      end

      if distance < 0
        fail ArgumentError, 'invalid value for "distance", must be greater than or equal to 0.'
      end
    ...
    def hash
      [place_id, location, categories, name, distance].hash
    end
