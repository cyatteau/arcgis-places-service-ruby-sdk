  ...
  class Rating
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Rating` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Rating`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@user.nil? && @user > 5
        invalid_properties.push('invalid value for "user", must be smaller than or equal to 5.')
      end

      if !@user.nil? && @user < 0
        invalid_properties.push('invalid value for "user", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      price_validator = EnumAttributeValidator.new('String', ["cheap", "moderate", "expensive", "veryExpensive"])
      return false unless price_validator.valid?(@price)
      return false if !@user.nil? && @user > 5
      return false if !@user.nil? && @user < 0
      true
    end
    ...
    def price=(price)
      validator = EnumAttributeValidator.new('String', ["cheap", "moderate", "expensive", "veryExpensive"])
      unless validator.valid?(price)
        fail ArgumentError, "invalid value for \"price\", must be one of #{validator.allowable_values}."
      end
      @price = price
    end
    ...
    def user=(user)
      if !user.nil? && user > 5
        fail ArgumentError, 'invalid value for "user", must be smaller than or equal to 5.'
      end

      if !user.nil? && user < 0
        fail ArgumentError, 'invalid value for "user", must be greater than or equal to 0.'
      end
