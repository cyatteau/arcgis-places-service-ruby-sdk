...
module OpenapiClient
  # Lists the opening hours of this place or POI along with the popular or busy hours.   A string is also provided that can be used for display. 
  class Hours
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Hours` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Hours`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'popular')
        self.popular = attributes[:'popular']
      end

      if attributes.key?(:'opening_text')
        self.opening_text = attributes[:'opening_text']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@opening_text.nil? && @opening_text.to_s.length < 1
        invalid_properties.push('invalid value for "opening_text", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@opening_text.nil? && @opening_text.to_s.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] opening_text Value to be assigned
    def opening_text=(opening_text)
      if !opening_text.nil? && opening_text.to_s.length < 1
        fail ArgumentError, 'invalid value for "opening_text", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [opening, popular, opening_text].hash
    end
