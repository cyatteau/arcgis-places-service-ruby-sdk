  ...
  class SocialMedia
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SocialMedia` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SocialMedia`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@facebook_id.nil? && @facebook_id.to_s.length < 1
        invalid_properties.push('invalid value for "facebook_id", the character length must be great than or equal to 1.')
      end

      if !@twitter.nil? && @twitter.to_s.length < 1
        invalid_properties.push('invalid value for "twitter", the character length must be great than or equal to 1.')
      end

      if !@instagram.nil? && @instagram.to_s.length < 1
        invalid_properties.push('invalid value for "instagram", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@facebook_id.nil? && @facebook_id.to_s.length < 1
      return false if !@twitter.nil? && @twitter.to_s.length < 1
      return false if !@instagram.nil? && @instagram.to_s.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] facebook_id Value to be assigned
    def facebook_id=(facebook_id)
      if !facebook_id.nil? && facebook_id.to_s.length < 1
        fail ArgumentError, 'invalid value for "facebook_id", the character length must be great than or equal to 1.'
      end
    ...
    def twitter=(twitter)
      if !twitter.nil? && twitter.to_s.length < 1
        fail ArgumentError, 'invalid value for "twitter", the character length must be great than or equal to 1.'
      end
    ...
    def instagram=(instagram)
      if !instagram.nil? && instagram.to_s.length < 1
        fail ArgumentError, 'invalid value for "instagram", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [facebook_id, twitter, instagram].hash
    end
